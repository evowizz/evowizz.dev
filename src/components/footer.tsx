...
const footerLinkCorners = [
  'rounded-tl-2xl rounded-tr-[4px] rounded-br-[4px] rounded-bl-[4px]',
  'rounded-tl-[4px] rounded-tr-2xl rounded-br-[4px] rounded-bl-[4px]',
  'rounded-tl-[4px] rounded-tr-[4px] rounded-br-[4px] rounded-bl-2xl',
  'rounded-tl-[4px] rounded-tr-[4px] rounded-br-2xl rounded-bl-[4px]',
] as const
...
const FooterLink = ({ href, children, className }: { href: string; children: ReactNode; className?: string }) => (
  <Link
    href={href}
    className={cn(
      'hover:text-inverse-primary motion-effects-default focus-ring-inverse-primary transition-colors',
      className,
    )}
  >
    {children}
            ...
            {footerDestinations.map((destination, index) => (
              <li key={destination.path}>
                <FooterLink
                  ...
                  className={cn(
                    'bg-inverse-on-surface/8 hover:bg-inverse-on-surface/15 flex h-full items-center justify-between gap-4 px-5 py-5 font-semibold transition-[color,background-color,border-radius] hover:rounded-3xl',
                    footerLinkCorners[index],
                  )}
                >
                  {destination.label}
